	...
	def align(self, seq1, seq2, banded, align_length):
		...
		if banded:
			...
			if too_big:
				...
				# This should be O(n) where n is alignment length – reverse traverses through the table to build the
				# 	alignment string.
				while True:
			...
			counter = 5
			# This double for gives us O(mn) where m is the length of one string and n is the length of the second
			for i in range(align_length + 1):
			...
			# O(n) where n is the length of the longer string (or the alignment length)
			while True:
